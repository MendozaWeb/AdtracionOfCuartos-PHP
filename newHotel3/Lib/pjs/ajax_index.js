...
function getIncr() {
    var auto = document.getElementById("btnenv").value;
    document.getElementById(lbl).innerHTML = data;
}

function getLogin(URL) {
    var btn = document.getElementById("btnAcep").value;
    var get_url = URL + "?btnAcep=" + btn; // El script a dónde se realizará la petición.
    $.ajax({
        type: "GET",
        url: get_url,
        data: $("#formulario").serialize(), // Adjuntar los campos del formulario enviado.
